  ...
  return (
    <main className="min-h-screen flex flex-col items-center justify-between p-6 selection:bg-white selection:text-black relative">
      ...
      <div className="w-full flex-1 flex flex-col items-center justify-center max-w-md space-y-12">
        <header className="text-center space-y-4 flex flex-col items-center">
          <div className="flex items-center gap-4">
            ...
            <a href="https://github.com/Ti-03/Chronos" target="_blank" rel="noopener noreferrer" className="text-neutral-500 hover:text-white">
        ...
        <section className="space-y-8 w-full">
            ...
            <div className="space-y-6">
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  <button onClick={() => setYearViewLayout('months')} className={`flex-1 py-3 text-xs uppercase ${yearViewLayout === 'months' ? 'bg-white text-black' : 'bg-neutral-900 text-neutral-500'}`}>Months</button>
                  <button onClick={() => setYearViewLayout('days')} className={`flex-1 py-3 text-xs uppercase ${yearViewLayout === 'days' ? 'bg-white text-black' : 'bg-neutral-900 text-neutral-500'}`}>Days</button>
                </div>
              </div>
              {yearViewLayout === 'days' && (
                <div className="space-y-2">
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => setDaysLayoutMode('continuous')} className={`flex-1 py-3 text-xs uppercase ${daysLayoutMode === 'continuous' ? 'bg-white text-black' : 'bg-neutral-900 text-neutral-500'}`}>Continuous</button>
                    <button onClick={() => setDaysLayoutMode('calendar')} className={`flex-1 py-3 text-xs uppercase ${daysLayoutMode === 'calendar' ? 'bg-white text-black' : 'bg-neutral-900 text-neutral-500'}`}>Calendar</button>
                  </div>
                </div>
              )}
            </div>
          ...
          <button
            ...
            className={`w-full py-4 text-sm uppercase tracking-widest ${isFormComplete ? 'bg-white text-black' : 'bg-neutral-900 text-neutral-600'}`}
          >
            Generate
          </button>
        </section>
        ...
        {wallpaperUrl && (
          <section className="w-full space-y-4 animate-in fade-in duration-500">
            <div className="flex items-center gap-2 p-4 border border-white/10 bg-white/5 rounded backdrop-blur-sm">
              ...
              <button onClick={copyToClipboard} className="text-xs text-white uppercase">{copied ? 'Copied' : 'Copy'}</button>
            </div>
            <div className="text-center">
              <a href={wallpaperUrl} target="_blank" rel="noopener noreferrer" className="text-xs text-neutral-500 border-b border-transparent hover:border-white pb-0.5">Preview</a>
            </div>
      ...
      <footer className="w-full flex flex-col items-center gap-2 py-8 mt-8 border-t border-white/5">
        <a href="https://github.com/ch1tmdgus" target="_blank" rel="noopener noreferrer" className="text-[10px] text-neutral-600 uppercase tracking-widest">Forked by Ch1tmdgus</a>
        <a href="https://github.com/Ti-03" target="_blank" rel="noopener noreferrer" className="text-[10px] text-neutral-600 uppercase tracking-widest">Created by Qutibah Ananzeh</a>
      </footer>
    </main>
  );
}
